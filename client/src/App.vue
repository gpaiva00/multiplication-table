<template>
  <v-app>
    ...
    <div class="text-center ma-2">
      <v-snackbar v-model="snack.show" :color="snack.color" :timeout="snack.timeout">
        <span v-html="snack.text"></span>
        <v-btn :color="snack.closeButtonColor" text @click="snack.show = false">Fechar</v-btn>
      </v-snackbar>
    </div>
  ...
  methods: {
    ...
    async createAccount(player) {
      ...
      const usernameExists =
        (await TabuadasRepository.checkUsername(player.username).then(
          d => d.length
        )) > 0;
    ...
    loginIn({ player, alreadyLoggedIn }) {
      const { _id, name, username, password, score, difficult = "" } =
        player || {};
